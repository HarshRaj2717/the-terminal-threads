...
class ImportFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        encrypt_window_button = tk.Button(
            self,
        ...
        encrypt_window_button.pack(before=mask_frame, side="bottom", anchor='s', fill='y', pady=10)
...
class EncryptFrame(tk.Frame):
    ...
    def load_and_encrypt(self):
        self.maskImage.config(image=cached_images["mask_image"],width=100)
        self.secretImage.config(image=cached_images["secret_image"],width=100)
        imagehandling = ImageHandler(1)
        imagehandling.encode_image(cached_imgpaths["secret_image"], cached_imgpaths["mask_image"])
        path = "samples\output.png"
        cached_images["output_image"] = ImageTk.PhotoImage(image=Image.open(path).resize((240,135)))
        cached_imgpaths["output_image"] = path
        self.stegImage.config(image=cached_images["output_image"], height=200, width=200)
